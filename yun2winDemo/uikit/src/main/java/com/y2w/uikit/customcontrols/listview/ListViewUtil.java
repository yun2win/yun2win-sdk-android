...
public class ListViewUtil {
	...
	//index是items的index，不包含header
	public static Object getGridViewHolderByIndex(GridView gridView, int index) {
		int firstVisibleFeedPosition = gridView.getFirstVisiblePosition();
		int lastVisibleFeedPosition = gridView.getLastVisiblePosition();

		//只有获取可见区域的
		if (index >= firstVisibleFeedPosition && index <= lastVisibleFeedPosition) {
			View view = gridView.getChildAt(index - firstVisibleFeedPosition);
			Object tag = view.getTag();
			return tag;
		} else {
			return null;
