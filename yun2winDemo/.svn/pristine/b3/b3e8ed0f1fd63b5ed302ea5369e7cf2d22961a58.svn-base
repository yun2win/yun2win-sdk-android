...
public class AutoRefreshListView extends ListView {
    ...
    private void addRefreshView(Context context) {
        refreshHeader = (ViewGroup) View.inflate(context, R.layout.lyy_listview_refresh, null);
        addHeaderView(refreshHeader, null, false);
        refreshFooter = (ViewGroup) View.inflate(context, R.layout.lyy_listview_refresh, null);
     ...
     * 加载完成
     */
    public void onRefreshComplete(int count, int requestCount, boolean needOffset,int addsety) {
        state = State.RESET;
        ...
        if (currentMode == Mode.START) {
            setSelectionFromTop(count + getHeaderViewsCount(), refreshableStart ? (offsetY+addsety) : 0);
        }
    }

    private void resetRefreshView(int count, int requestCount) {
        boolean ismore=(count==requestCount);
        if (currentMode == Mode.START) {
            ...
            if (getCount() == count + getHeaderViewsCount() + getFooterViewsCount()) {
                refreshableStart = ismore;
            } else {
                refreshableStart = ismore;
            }
        } else {
            refreshableEnd = ismore;
        }
        updateRefreshView();
    }
