				...
				if(msg.what == 1){
					...
					if(mUpdate != null){
						if(curVersionCode < mUpdate.getVersionCode()){
							...
							File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/ytw/Log/version_" + mUpdate.getVersionCode());
							long length=mUpdate.getSize();
							DecimalFormat df = new DecimalFormat("0.00");
					    	//进度条下面显示的总文件大小
							updateMsg +="\n 安装包大小："+df.format((float) length / 1024 / 1024) + "MB\n";
		...
		public void run() {
			try {
				...
				if(storageState.equals(Environment.MEDIA_MOUNTED)){
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/ytw/Update/";
				...
				HttpURLConnection conn = HttpsClient.getHttpURLConnection(url);
				conn.connect();
