...
public class AnimTabLayout extends LinearLayout implements OnClickListener{
	...
	private void initView(){
		...
		mTabContainer.setBackgroundResource(R.color.topbar_bg);
		LayoutParams params = new LayoutParams(LayoutParams.FILL_PARENT,LayoutParams.FILL_PARENT);
		params.weight = 1;
		addView(mTabContainer,params);

		mIndicatorLayout = new LinearLayout(mContext);
		//mIndicatorLayout.setBackgroundColor(Color.GREEN);
		addView(mIndicatorLayout,new LayoutParams(LayoutParams.FILL_PARENT,7));
		mIndicator = new ImageView(mContext);
		mIndicator.setBackgroundColor(Color.parseColor("#1cc09f"));
	...
	public void setAdapter(BaseAdapter adapter){
		...
		for (int i = 0; i < mTabCount; i++) {
			...
			child.setTag(R.color.topbar_bg, i);
	...
	public void onClick(View v) {
		final int index = (Integer) v.getTag(R.color.topbar_bg);
