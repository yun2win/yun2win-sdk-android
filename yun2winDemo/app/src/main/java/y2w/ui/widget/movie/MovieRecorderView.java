...
public class MovieRecorderView extends LinearLayout implements MediaRecorder.OnErrorListener {
    ...
    private void createRecordDir() {
        //录制的视频保存文件夹
        File sampleDir = new File(Config.CACHE_PATH_MOVIE);//录制视频的保存地址
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        ...
        // 创建文件
        try {
            mVecordFile = File.createTempFile("y2wMovie", ".mp4", vecordDir);// mp4格式的录制的视频文件
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public String getmVecordPath() {
        return mVecordFile.getPath();
    }
